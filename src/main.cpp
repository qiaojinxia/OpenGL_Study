...
#include "loader/BinaryFileReader.h"
...
int main() {
    auto data = BinaryFileReader::open_file("/Users/cboy/Downloads/OpenGL_Study/src/reflect/Layer_Fb.bfbs");
    if (!data.empty()) {
        static const reflection::Schema &schema = *reflection::GetSchema(data.data());
        static auto enums = schema.enums();

        const reflection::Enum *enum_ = nullptr;

        enum_ = enums->LookupByKey("rendering.RenderTarget");
